...
これまでに行った修正と、その判断理由は [CHANGELOG.md](CHANGELOG.md) にまとめてある。
「なぜこのルールがあるのか」を辿りたいときはそちらを見ること。

（週次ジョブによる `strategy.md` の自動更新は [strategy.md](strategy.md) の
「変更履歴」セクションに記録される）
...
柱ごとに投稿の「型」が違う。詳細は [content_prompt.md](content_prompt.md) の「発信の柱」を参照。

1. 即実践プロンプト例 — プロンプトを載せる
2. AIツール紹介
3. あるある＋即Tips
4. よくある失敗と改善策 — プロンプトを載せる
5. AIの落とし穴と対処
6. 業務別時短Tips
7. 数字で示す効果

この順番は `scripts/post.py` の `PILLAR_ROTATION` で定義しており、投稿数から
機械的に決まる。プロンプトを載せる柱が連続しないように並べてある。

柱の名前は `seeds.jsonl` / `build_seeds.py` / `post.py` / `content_prompt.md` の
4箇所で一致している必要がある（ズレると巡回が空振りする。post.py が起動時に警告を出す）。

### 投稿の種（seeds.jsonl）

`発信の柱(7) × 業務シーン(28) × 詰まりポイント(7) = 1372通り` をシャッフルしてキューにしている。
1日3投稿なので約455日分。使い切ったら `build_seeds.py` を作り直すか、軸を足す。

種は「柱のローテーションに合う未使用のもの」から選ばれる。あわせて直近4投稿と
同じ業務シーンは避ける。
