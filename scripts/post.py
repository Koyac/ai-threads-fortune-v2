"""
日次ジョブ本体。GitHub Actions から1日2回呼ばれ、そのたびに実行される。
...
# GitHub Actions 側で15分早く起動してランダム待機することで、実投稿はlabelの前後15分
# あたりに散る。slotにはlabelを記録し、週次集計が細かい時刻で割れないようにする。
...
- 本文は100〜300文字。数えて超えていたら削ってから出力すること
